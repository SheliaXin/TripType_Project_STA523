...
First, we looked at different ways to visualize the datasets. The Shiny App found in ui.R and server.R contains three different plots where various inputs can be changed, and the plots will automatically change. In all three plots, the dataset can either be chosen as the training set or the testing set. The step that takes the longest to load in the App is selecting one of these two datasets because they have almost 700,000 rows each. Descriptions of the three different plots are as follows:
...
The data was cleaned and organized in order to use it better for a classification algorithm. The columns for each visit number are now: VisitNumer, number of purchases, number of returns, weekday, the departments for the trip, and fine line numbers for the trip, which is a more descreptive categorizing number for each item. 

##Classification and Prediction 

Principal Component Analysis on different departments
