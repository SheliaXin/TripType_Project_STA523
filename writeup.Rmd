...
To classify the visit number, we try SVM algorithm, 
...
First, we use Cross Validation to set our parameter `cost`, `gamma` in SVM model. We sample $80\%$ data(in train set) to train and the left $20\%$ to test. With testing gamma from 1e-6 to 1e-1, cost from 10 to 100, we then select the best choice (gamma = , cost = ) as our parameter.

Second, we build up our SVM models using train set. With using "linear" kernel and the misclassification error is . With using "linear" kernel and the misclassification error is . 

Third, we use both the two SVM models above to predict the trip type in test set and upload our result to Kaggle. The model with "linear" kernel gets point and the model with "radial" kernel gets point.
