...
To classify the trips and the visit numbers, we used SVM algorithm.
...
To improve our Support Vector Machine Models, we could use tuning to set our parameter `cost`, `gamma` in SVM model. We could also try to incorporate the FineLineNumber into our model. We first attempted to try to incorporate it, but it wasn't working with our framework. There are also possibly other variables that could be provided to make our model better at classifying on predicting.

This project further practice our R skills, helped with learning about classification algorithms, and furthured improved our knowledge of using the Shiny App. 
